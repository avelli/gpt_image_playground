...
const IMAGE_MENTION_PATTERN = /^@图(\d+)$/
...
export function resolvePromptMentionedImageIds(prompt: string, inputImages: InputImage[]) {
  const ids: string[] = []
  const seen = new Set<string>()
  for (const token of prompt.match(/@图\d+/g) ?? []) {
    const match = token.match(IMAGE_MENTION_PATTERN)
    const index = match ? Number(match[1]) - 1 : -1
    const id = inputImages[index]?.id
    if (id && !seen.has(id)) {
      seen.add(id)
      ids.push(id)
    }
